...
#
# A full featured zsh base configuration in a simple include
#
# single file install:
...
# ------------------------------------------------------------------------------
_features=
if [[ -z "$ZEBRAFISH_DIR" ]]; then
  if [[ -n "$ZDOTDIR" ]]; then
    ZEBRAFISH_DIR="$ZDOTDIR"
  else
    ZEBRAFISH_DIR="${XDG_CONFIG_DIR:-$HOME/.config}/zebrafish"
  fi
fi
[[ -d "$ZEBRAFISH_DIR" ]] || mkdir -p "$ZEBRAFISH_DIR"
